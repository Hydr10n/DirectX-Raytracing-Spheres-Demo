...
export {
	struct Streamline {
		static constexpr Feature Features[]{ kFeatureDLSS, kFeatureDLSS_G, kFeatureDLSS_RR, kFeatureNIS, kFeaturePCL, kFeatureReflex };

		Streamline(const Streamline&) = delete;
		Streamline& operator=(const Streamline&) = delete;

		Streamline(CommandList& commandList, uint32_t viewportID) :
			m_commandList(commandList), m_viewport(viewportID) {
			if (_.IsInitialized) {
				for (const auto feature : Features) {
					auto LUID = commandList.GetDeviceContext().Device->GetAdapterLuid();
					AdapterInfo adapterInfo;
					adapterInfo.deviceLUID = reinterpret_cast<uint8_t*>(&LUID);
					adapterInfo.deviceLUIDSizeInBytes = sizeof(LUID);
					if (slIsFeatureSupported(feature, adapterInfo) == Result::eOk) {
						m_features.emplace(feature);
					}
				}
			}
		}
		...
		auto IsAvailable(Feature feature) const { return m_features.contains(feature); }
		...
		auto SetConstants(const T& constants) const {
			if constexpr (is_same_v<T, Constants>) {
				return slSetConstants(constants, *m_frameToken, m_viewport);
			}

			if constexpr (is_same_v<T, DLSSDOptions>) {
				return slDLSSDSetOptions(m_viewport, constants);
			}

			if constexpr (is_same_v<T, DLSSGOptions>) {
				return slDLSSGSetOptions(m_viewport, constants);
			}

			if constexpr (is_same_v<T, DLSSOptions>) {
				return slDLSSSetOptions(m_viewport, constants);
			}

			if constexpr (is_same_v<T, NISOptions>) {
				return slNISSetOptions(m_viewport, constants);
			}

			throw;
		}

		auto Evaluate(Feature feature, span<const ResourceTag> resourceTags = {}) const {
			vector<const BaseStructure*> inputs{ &m_viewport };
			for (const auto& resourceTag : resourceTags) {
				inputs.emplace_back(&resourceTag);
			}
...
auto CreateResourceTags(span<ResourceTagDesc> resourceTagDescs) {
	...
	for (auto& resourceTagDesc : resourceTagDescs) {
		resourceTags.emplace_back(resourceTagDesc);
	}
	return resourceTags;
}
