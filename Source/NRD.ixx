...
export struct NRD {
	...
	NRD(
		...
		if (const QueueFamilyD3D12Desc queueFamilyDesc{ .d3d12Queues = &deviceContext.CommandQueue, .queueNum = 1 };
			nriCreateDeviceFromD3D12Device(
				{
					.d3d12Device = deviceContext,
			...
			const IntegrationCreationDesc integrationCreationDesc{
			...
			const InstanceCreationDesc instanceCreation{
				.denoisers = data(denoiserDescs),
				.denoisersNum = static_cast<uint32_t>(size(denoiserDescs))
			};
			if ((m_isAvailable = m_integration.Initialize(integrationCreationDesc, instanceCreation, *m_device, m_NRI, m_NRI))) {
				m_textureBarrierDescs.resize(size(m_userPool));
			}
		}
	}
	...
	auto Tag(ResourceType type, DirectX::Texture& texture) {
		...
		if (textureBarrierDesc.texture != nullptr) {
			if (m_NRI.GetTextureNativeObject(*textureBarrierDesc.texture) == reinterpret_cast<uint64_t>(texture.GetNative())) {
				return true;
			}
