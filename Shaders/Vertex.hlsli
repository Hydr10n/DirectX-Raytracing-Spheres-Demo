...
struct VertexDesc {
	uint Stride;
	uint3 _;
	struct { uint Normal, TextureCoordinate, Tangent, _; } Offsets;
	...
	float3 LoadNormal(ByteAddressBuffer buffer, uint index) { return asfloat(buffer.Load3(Stride * index + Offsets.Normal)); }
	...
	float2 LoadTextureCoordinate(ByteAddressBuffer buffer, uint index) { return asfloat(buffer.Load2(Stride * index + Offsets.TextureCoordinate)); }
	...
	float3 LoadTangent(ByteAddressBuffer buffer, uint index) { return asfloat(buffer.Load3(Stride * index + Offsets.Tangent)); }
