...
namespace Math {
	float2 CalculateUV(float2 pixelPosition, float2 pixelDimensions, float2 cameraJitter = 0) { return (pixelPosition + 0.5f + cameraJitter) / pixelDimensions; }

	float2 CalculateNDC(float2 UV) { return UV * float2(2, -2) + float2(-1, 1); }
	...
	float ToSolidAnglePDF(float AreaPDF, float _length, float cosTheta) { return AreaPDF * _length * _length / cosTheta; }
}
