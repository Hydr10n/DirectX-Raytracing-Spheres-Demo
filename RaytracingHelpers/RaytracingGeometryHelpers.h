...
namespace RaytracingHelpers {
	namespace Geometries {
		...
		class Triangles : public Geometry {
			static_assert(sizeof(Vertex) >= sizeof(float) * 3, "Unsupported vertex format");
			static_assert(sizeof(Index) == 2 || sizeof(Index) == 4, "Unsupported index format");
		...
		public:
			Triangles(
				...
				m_geometryDesc.Triangles = {
					...
					.IndexFormat = sizeof(Index) == 2 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT,
			...
			void CreateShaderResourceViews(
				ID3D12Device* pDevice,
				D3D12_CPU_DESCRIPTOR_HANDLE vertexDescriptor, D3D12_CPU_DESCRIPTOR_HANDLE indexDescriptor
			) const {
				...
				pDevice->CreateShaderResourceView(m_vertexBuffer.Get(), &srvDesc, vertexDescriptor);

				srvDesc.Format = DXGI_FORMAT_R32_TYPELESS;
				srvDesc.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_RAW;
				srvDesc.Buffer.NumElements = m_geometryDesc.Triangles.IndexCount / sizeof(Index);
				srvDesc.Buffer.StructureByteStride = 0;
				pDevice->CreateShaderResourceView(m_indexBuffer.Get(), &srvDesc, indexDescriptor);
			}
