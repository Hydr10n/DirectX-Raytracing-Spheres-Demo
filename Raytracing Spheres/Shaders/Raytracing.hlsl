...
void RayGeneration() {
	...
	for (uint i = 0; i < g_sceneConstant.AntiAliasingSampleCount; i++) {
		color += TracePrimaryRay(GenerateCameraRay(raysIndex, raysDimensions, g_sceneConstant.CameraPosition.xyz, g_sceneConstant.ProjectionToWorld, random.Float()), MAX_TRACE_RECURSION_DEPTH, random);
	}
