...
inline RayDesc GenerateCameraRay(uint3 raysIndex, uint3 raysDimensions, float3 origin, float4x4 projectionToWorld, float offset = 0.5f) {
	const float2 screenPosition = (raysIndex.xy + offset) / raysDimensions.xy * float2(2, -2) + float2(-1, 1);
