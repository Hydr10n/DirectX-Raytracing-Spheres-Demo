...
LRESULT MainWindow::OnMessageReceived(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CONTEXTMENU: {
		...
		for (size_t size = m_displayResolutions.size(), i = 0; i < size; i++) {
			AppendMenuW(hMenuResolution, MF_STRING | (m_windowModeHelper.ClientSize == m_displayResolutions[i] ? MF_CHECKED : MF_UNCHECKED), static_cast<UINT_PTR>(static_cast<size_t>(MenuID::FirstResolution) + i), (to_wstring(m_displayResolutions[i].cx) + L" × " + to_wstring(m_displayResolutions[i].cy)).c_str());
		}
