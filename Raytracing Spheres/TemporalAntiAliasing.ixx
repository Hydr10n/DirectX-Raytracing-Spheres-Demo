...
export namespace DirectX::PostProcess {
	struct TemporalAntiAliasing : IPostProcess {
		SIZE TextureSize{};
		struct { D3D12_GPU_DESCRIPTOR_HANDLE MotionsSRV, HistoryOutputSRV, CurrentOutputSRV, FinalOutputUAV; } Descriptors{};
		...
		void Process(ID3D12GraphicsCommandList* commandList) noexcept override {
			...
			commandList->SetComputeRootDescriptorTable(0, Descriptors.MotionsSRV);
			commandList->SetComputeRootDescriptorTable(1, Descriptors.HistoryOutputSRV);
			commandList->SetComputeRootDescriptorTable(2, Descriptors.CurrentOutputSRV);
			commandList->SetComputeRootDescriptorTable(3, Descriptors.FinalOutputUAV);
